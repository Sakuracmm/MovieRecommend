...
//    ratingTmp3.createTempView("v_ratings3")
    ...
    spark.sql(s"create table trainingDataAsc as select userId, movieId, rating from v_ratings2 limit ${trainingDataCount/3}")
    spark.sql(s"insert into table trainingDataAsc select userId, movieId, rating from v_ratings4")
    spark.sql(s"insert into table trainingDataAsc select userId, movieId, rating from v_ratings5")
    ...
    spark.sql(s"create table trainingDataDesc as select userId, movieId, rating from v_ratingDesc2 limit ${testDataCount/3}")
    spark.sql(s"insert into table trainingDataDesc select userId, movieId, rating from v_ratingDesc3")
    spark.sql(s"insert into table trainingDataDesc select userId, movieId, rating from v_ratingDesc4")
