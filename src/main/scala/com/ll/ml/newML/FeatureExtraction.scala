...
/**
  ...
  */
object FeatureExtraction extends AppConf{

//  val spark = SparkSession.builder.master("spark://slave1:7077").appName("FeatureExtraction").getOrCreate()
  ...
  def getFeatures(): DataFrame = {
    ...
    val ratings = spark.read.textFile("hdfs://slave1:9000/newMovieRecommendData/ratings.dat").map(parseRating).toDF()
